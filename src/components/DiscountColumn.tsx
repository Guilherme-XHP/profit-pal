        ...
        <ul className="flex-1 min-h-0 space-y-1 overflow-y-auto pr-1">
          {rows.map((row, idx) => (
            <li
              key={`${row.percent}-${idx}`}
              className={cn(
                "flex items-center justify-between rounded-lg border bg-card/60 px-2.5 py-1.5 transition-colors",
                s.row,
              )}
            >
              <span className={cn("text-xs font-bold tabular-nums", s.accent)}>
